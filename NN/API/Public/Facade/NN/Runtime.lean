...
abbrev eval : NN.API.TorchLean.LayerCore.Mode :=
  .eval

/-- Training-mode layer behavior, matching PyTorch's `model.train()` concept. -/
abbrev train : NN.API.TorchLean.LayerCore.Mode :=
  .train
...
abbrev forward {σ τ : Shape}
    ...
    (opts : Options)
    (mode : NN.API.TorchLean.LayerCore.Mode)
  ...
  NN.API.TorchLean.LayerCore.Seq.forward (α := α) opts mode model params x
...
abbrev predict {σ τ : Shape}
    ...
    (params : NN.API.TorchLean.ParamList α (paramShapes model))
    (x : Tensor.T α σ) : IO (Tensor.T α τ) :=
  NN.API.TorchLean.LayerCore.Seq.predict (α := α) opts model params x
...
def compile {σ τ : Shape} (model : Sequential σ τ)
  ...
  let artifact ← NN.API.TorchLean.LayerCore.Seq.compileForward (α := α) model
...
def compileWithMode {σ τ : Shape}
    ...
    {α : Type} [Runtime.TensorScalar α] [DecidableEq Shape] :
    IO (Compiled (paramShapes model) σ τ α) := do
  let artifact ← NN.API.TorchLean.LayerCore.Seq.compileForwardWithMode (α := α) mode model
  pure { artifact := artifact }
...
abbrev scalarModuleDef {σ τ : Shape} (model : Sequential σ τ)
    ...
    NN.API.TorchLean.Module.ScalarModuleDef (paramShapes model) [σ, τ] :=
...
def compile {σ τ : _root_.Spec.Shape}
    ...
    IO (_root_.TorchLean.nn.Compiled (NN.API.nn.paramShapes model) σ τ α) :=
