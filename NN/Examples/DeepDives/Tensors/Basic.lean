...
/-!
# Deep-Dive Tensor Basics

This file is for tensor work just below the public quickstart layer. The examples cover the
operations people ask about once typed shapes make sense:
...
back with `toTensor` makes it usable by spec/proof layer code and by the tensor widgets.
-/
