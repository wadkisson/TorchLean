...
`IEEE32Exec` is the executable, proof-relevant bit-level IEEE-754 binary32 kernel. The common finite
path decodes to exact intermediates and rounds once; values with an all-ones exponent field then use
the NaN and infinity rules collected here.
...
/-- `chooseNaN2` returns `none` only when neither operand is a NaN. -/
theorem not_isNaN_of_chooseNaN2_none (x y : IEEE32Exec) (h : chooseNaN2 x y = none) :
  ...
  all_goals
    ...
    cases hxS : isSNaN x <;> cases hyS : isSNaN y <;>
      simp [chooseNaN2, hxS, hyS, hx, hy] at h
...
When `chooseNaN*` selects a NaN, the corresponding operation returns that selected value. These
lemmas expose the propagation behavior so higher-level proofs can avoid unfolding the full
operation definitions, regardless of whether an operation checks its finite path first.
-/

private lemma isNaN_left_or_right_of_chooseNaN2_some (x y nan : IEEE32Exec)
    (h : chooseNaN2 x y = some nan) : isNaN x = true ∨ isNaN y = true := by
  cases hx : isNaN x with
  | false =>
      cases hy : isNaN y with
      | false => simp [chooseNaN2, isSNaN, hx, hy] at h
      | true => simp
  | true => simp
...
theorem add_eq_of_chooseNaN2_some (x y nan : IEEE32Exec) (h : chooseNaN2 x y = some nan) :
    add x y = nan := by
  rcases isNaN_left_or_right_of_chooseNaN2_some x y nan h with hx | hy
  · simp [add, toDyadic?_eq_none_of_isNaN hx, h]
  · simp [add, toDyadic?_eq_none_of_isNaN hy, h]
...
theorem mul_eq_of_chooseNaN2_some (x y nan : IEEE32Exec) (h : chooseNaN2 x y = some nan) :
    mul x y = nan := by
  rcases isNaN_left_or_right_of_chooseNaN2_some x y nan h with hx | hy
  · simp [mul, toDyadic?_eq_none_of_isNaN hx, h]
  · simp [mul, toDyadic?_eq_none_of_isNaN hy, h]
...
theorem div_eq_of_chooseNaN2_some (x y nan : IEEE32Exec) (h : chooseNaN2 x y = some nan) :
    div x y = nan := by
  rcases isNaN_left_or_right_of_chooseNaN2_some x y nan h with hx | hy
  · simp [div, toDyadic?_eq_none_of_isNaN hx, h]
  · simp [div, toDyadic?_eq_none_of_isNaN hy, h]
...
/-! ## Exception-status rules -/

/-- Adding infinities with opposite signs raises the invalid-operation indicator. -/
theorem addWithStatus_invalid_of_opposite_infinities (x y : IEEE32Exec)
    (hx : isInf x = true) (hy : isInf y = true) (hsign : (signBit x != signBit y) = true) :
    (addWithStatus x y).status.invalid = true := by
  have hxNaN : isNaN x = false := by
    simp [isNaN, isInf] at hx ⊢
    intro _
    exact hx.2
  have hyNaN : isNaN y = false := by
    simp [isNaN, isInf] at hy ⊢
    intro _
    exact hy.2
  simp [addWithStatus, hx, hy, hxNaN, hyNaN, hsign, isSNaN]

/-- Dividing a finite nonzero value by zero raises divide-by-zero, not invalid. -/
theorem divWithStatus_divideByZero_of_finite_nonzero (x y : IEEE32Exec)
    (hxFinite : isFinite x = true) (hxZero : isZero x = false) (hyZero : isZero y = true) :
    (divWithStatus x y).status.divideByZero = true ∧
      (divWithStatus x y).status.invalid = false := by
  have hxExp : expField x ≠ expAllOnes := by
    exact (bne_iff_ne).mp hxFinite
  have hxNaN : isNaN x = false := by
    simp [isNaN, hxExp]
  have hxInf : isInf x = false := by
    simp [isInf, hxExp]
  have hyNaN : isNaN y = false := by
    have hyFields : expField y = 0 ∧ fracField y = 0 := by
      simpa [isZero, Bool.and_eq_true] using hyZero
    have hzeroExp : (0 : UInt32) ≠ expAllOnes := by decide
    simp [isNaN, hyFields.1, hzeroExp]
  obtain ⟨dx, hdx⟩ := exists_toDyadic?_of_isFinite hxFinite
  have hdy := toDyadic?_eq_zero_of_isZero hyZero
  have hdxMant : dx.mant ≠ 0 := by
    intro hmant
    have : isZero x = true :=
      isZero_eq_true_of_toDyadic?_some_of_mant_eq_zero hdx hmant
    simp [hxZero] at this
  simp [divWithStatus, hdx, hdy, hdxMant]

/-- Square root of a negative nonzero numeric value raises invalid. -/
theorem sqrtWithStatus_invalid_of_negative_nonzero (x : IEEE32Exec)
    (hxNaN : isNaN x = false) (hxSign : signBit x = true) (hxZero : isZero x = false) :
    (sqrtWithStatus x).status.invalid = true := by
  simp [sqrtWithStatus, hxNaN, hxSign, hxZero, isSNaN]
