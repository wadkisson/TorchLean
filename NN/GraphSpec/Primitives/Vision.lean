...
Output shape is `.dim (Spec.Shape.size s) .scalar`, i.e. a vector whose length is the number of
...
PyTorch analogy: `torch.flatten(x)`.
 -/
def flatten (s : Shape) : Primitive [] s (.dim (Spec.Shape.size s) .scalar) :=
  { name := "flatten"
    specFwd := fun {α} _ctx _params x =>
...
def batchnormChw
    ...
    Primitive
      [ .dim channels .scalar, .dim channels .scalar ]
