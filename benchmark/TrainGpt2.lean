...
Fastest wired TorchLean path on `cuda-train-defaults`:

* `--device cuda` + `-K cuda=true -K libtorch=true` eager runtime
* LibTorch SDPA for MHA (CUDA default when LibTorch is linked)
* integer token-id embedding gather (`embeddingBatchSeqNat`), not one-hot matmul
* storage-first `instantiateFloat` so params are allocated/initialized on the GPU
...
LEAN_PROFILE=1 BENCH_MAX_ITERS=3 LEAN_PROFILE_OUT=benchmark/out/torchlean-trace.json \
  lake -R -K cuda=true -K libtorch=true exe benchmark_gpt2 --device cuda
```
...
/--
Mirror of last week's ~7× `gpt2-500m` timing config (`Gpt500m.lean` / `run_benchmark.py`):
vocab 50257, seq 128, 32L × 1024d × 16H, batch 1 → ~506M params, TL ~604 ms vs PT ~87 ms.
-/
def vocab : Nat := 50257
...
At `BENCH_MAX_ITERS=10`, `sampleEvery=25` still fires at iter 0 and burns ~120 full
32-layer forwards (plus large logit D2H each) before any meaningful train timing.
...
partial def generate
  ...
  -- Params are typed as `embed :: bodyShapes`, so only `.cons` is possible.
  ...
  | .cons embedP bodyParams => do
      ...
      for stepIdx in [0:maxNew] do
        ...
        -- Repeat the same window across the batch axis (matches batch=1 model shape).
...
profiled def main (args : List String) : IO UInt32 :=
  Runtime.runFloat "benchmark_gpt2" args
    (banner := fun _ =>
      "benchmark_gpt2: gpt2-500m 7×-era config (32L/seq128/vocab50257) + LibTorch SDPA MHA + token-id gather")
    (k := fun opts rest => do
      ...
      nn.withModel bodyM fun body => do
        ...
        -- Storage-first token-id LM: module def owns zero placeholders + CUDA runtimeInit.
        let defn :=
          NN.API.nn.models.causalTransformerTokenIdLmScalarModuleDef cfg body
            (embedInitSeed := seed)
        let nParams :=
        ...
        IO.println "MHA: libtorch.sdpa_autograd (CUDA default when LibTorch is linked)"
        if !doSample then
        ...
        for it in [0:maxIters + 1] do
          ...
          if it != maxIters then
            ...
            -- Extra full forward only on long runs (short profiles skip it).
