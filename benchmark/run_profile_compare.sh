#!/usr/bin/env bash
# Profile TorchLean (LeanProfiler) vs PyTorch on the gpt2-500m config
# (32L × 1024d, seq 128, vocab 50257, batch 1).
#
# Lab harness only: stock CUDA build (no LibTorch / product hot-path patches).
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
cd "$ROOT"

# Keep this tiny by default: each iter is a full ~500M fwd+bwd+AdamW.
# Short runs also force accum=1 inside the Lean/Python harnesses.
